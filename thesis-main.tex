...
%inserisci il codice di richiamo a InputParameter, e completa la descrizione di App con cameracalibrator.java e camerapose.java
\lstinputlisting[float,language=Java,label={lst:opencv-loading}]{listings/opencv-loading.java}

\section{Calibrazione}
%descrivi la fase di calibrazione in maniera meticolosa, ponendo enfasi sulle parti in cui usi funzionalità di javacv e ottieni i parametri come la camera matrix
\section{Posa}
%descrivi la classe più importante del codice (questa) andando molto nel dettaglio di alcuni metodi (soprattutto calcPose) dopodichè dai un'idea generale del funzionamento di questa classe riportando i punti salienti
